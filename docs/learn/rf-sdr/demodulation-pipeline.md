...
redirect_from: /learn/demodulation-pipeline/
...
Turning [IQ samples](/learn/rf-sdr/iq-data/) into usable bits is a five-stage **pipeline**:
...
**(4) symbol recovery** times and slices it into [symbols](/learn/rf-sdr/digital-modulation/);
...
You've met the pieces — [IQ](/learn/rf-sdr/iq-data/), [modulation](/learn/rf-sdr/digital-modulation/),
[filtering](/learn/rf-sdr/filtering-decimation/). This lesson assembles them into the chain that
every signal passes through inside an SDR decoder. It's the detailed version of the
[antenna-to-audio](/learn/rf-sdr/antenna-to-audio/) story.
...
The captured [IQ](/learn/rf-sdr/iq-data/) covers a whole band, but your channel sits somewhere
...
narrow signal. This is the [filtering & decimation](/learn/rf-sdr/filtering-decimation/) step,
...
[modulation type](/learn/rf-sdr/digital-modulation/):

- **FM/FSK** — track the instantaneous frequency; for 4FSK (P25/DMR) the output is a
  stream of four levels.
- **PSK** — track the phase angle of each [IQ](/learn/rf-sdr/iq-data/) sample.
...
The demodulated stream has to be sliced into discrete [symbols](/learn/rf-sdr/digital-modulation/)
at exactly the right instants. That requires knowing the signal's rhythm —
**[clock recovery](/learn/rf-sdr/clock-recovery/)** — so the decoder samples each symbol at its
centre, where the [eye diagram](/learn/rf-sdr/digital-modulation/) is most open. Get the timing
...
[control-channel message](/learn/rf-sdr/what-is-trunking/), or [vocoder](/learn/rf-sdr/vocoders/) frames
...
fix. Because the [constellation and eye diagram](/learn/rf-sdr/tuning-with-scopes/) watch stage
3–4, they warn you *before* the decoded output fails, which is what makes them such good
[tuning tools](/learn/rf-sdr/tuning-with-scopes/).

## How GopherTrunk runs many pipelines at once

Since every stage is arithmetic on the shared [IQ stream](/learn/rf-sdr/iq-data/), GopherTrunk
instantiates **one pipeline per channel** — the [control channel](/learn/rf-sdr/what-is-trunking/)
plus each active [voice channel](/learn/rf-sdr/antenna-to-audio/) — and runs them concurrently.
...
| After decode | 9600 bits/s, then a handful of [vocoder](/learn/rf-sdr/vocoders/) frames |
