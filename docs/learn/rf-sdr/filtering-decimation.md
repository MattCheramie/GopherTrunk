...
redirect_from: /learn/filtering-decimation/
...
A wide [IQ capture](/learn/rf-sdr/sample-rate-nyquist/) contains far more than the one channel
...
decimate** — or out-of-band energy [aliases](/learn/rf-sdr/sample-rate-nyquist/) back in. Doing
this per channel ("channelising") is how one SDR feeds **many decoders at once**.
</div>

You've captured a couple of megahertz of [IQ](/learn/rf-sdr/iq-data/). Now you need one
...
[demodulation pipeline](/learn/rf-sdr/demodulation-pipeline/).
...
[demodulation](/learn/rf-sdr/demodulation-pipeline/), [clock recovery](/learn/rf-sdr/clock-recovery/),
...
still well above [Nyquist](/learn/rf-sdr/sample-rate-nyquist/) for a 12.5 kHz signal, but a
...
[Nyquist](/learn/rf-sdr/sample-rate-nyquist/) limit in reverse). If any energy still sits
...
[demodulation pipeline](/learn/rf-sdr/demodulation-pipeline/)**: shift the channel to centre,
filter it, decimate. Because it's all arithmetic on the shared
[IQ stream](/learn/rf-sdr/iq-data/), GopherTrunk runs **many of these in parallel** — one for
the [control channel](/learn/rf-sdr/what-is-trunking/) and one for each
[voice channel](/learn/rf-sdr/antenna-to-audio/) it's following — from a single capture. That
