---
slug: clock-recovery
redirect_from: /learn/clock-recovery/
...
[symbol](/learn/rf-sdr/digital-modulation/) is only a clean value for an instant at its
...
diagram](/learn/rf-sdr/digital-modulation/)**. When the signal is too weak or smeared, timing
...
[demodulation pipeline](/learn/rf-sdr/demodulation-pipeline/). It's why a signal with plenty of
strength can still fail to decode — and why the scopes are such powerful diagnostic
tools.

## Why symbol timing matters

A digital signal carries one [symbol](/learn/rf-sdr/digital-modulation/) after another at a
fixed [symbol rate](/learn/rf-sdr/symbols-and-baud/). But each symbol is only a clean, distinct
...
that's neither one thing nor the other — an error. At 4800 [baud](/learn/rf-sdr/symbols-and-baud/),
...
million. At 4800 [baud](/learn/rf-sdr/symbols-and-baud/) that's only about 0.1 symbol of slip
...
1. Makes an initial estimate of the symbol period (it knows the nominal
   [baud](/learn/rf-sdr/symbols-and-baud/)).
...
The [eye diagram](/learn/rf-sdr/digital-modulation/) makes timing visible. It overlays many
...
When the signal is too weak, too noisy, or smeared by [multipath](/learn/rf-sdr/propagation/),
the timing loop can't keep up and **loses lock**. Symbols get sampled at the wrong
moments, bit errors outrun [error correction](/learn/rf-sdr/demodulation-pipeline/), and the
...
lock](/learn/rf-sdr/tuning-with-scopes/) — often the fix is more SNR (better antenna/placement,
right [gain](/learn/rf-sdr/gain-and-agc/)) so the timing loop has a clean signal to track.
