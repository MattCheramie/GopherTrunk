...
| **MPT 1327 CWSC bit-error tolerance** | BCH(64, 48, 2) per-codeword check shipping; exact-match 16-bit Codeword Synchronisation Code (`1100010011010111`) detection shipping per the MPT 1327 standard (the "inter-codeword interleaver" previously listed here was a misnomer — MPT 1327 transmits 64-bit codewords back-to-back without inter-codeword bit permutation). Remaining follow-up: 1-2 bit error tolerance on the CWSC match for noisy on-air captures | Real-air capture for tolerance threshold calibration |
