...
| **Loudness normalization** | `recordings.normalize.enabled` | `false` | EBU R128 / BS.1770 per-call loudness normalization (default target -16 LUFS, true peak -1.5 dBTP), pure Go — no ffmpeg. Each finished WAV is measured and rewritten in place to a consistent perceptual level, so calls from different talkgroups/sources play back evenly. Opt-in because it rewrites the recorded file and not every operator wants level changes applied to their archive; live monitoring is unaffected. |
