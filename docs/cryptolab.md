...
keystream analysis, CRC reverse-engineering, analog voice descrambling — and
its flagship subject, the Motorola P25 talker-alias obfuscation recovery.

It is **clean-room**: everything is derived from observed data and public
structural description. Nothing is ported from a GPL decoder, and nothing
flips `motorola.CipherVerified` — the live alias decode stays gated.
...
|------|-------|--------------|
| `alias` | `gauge`, `structure`, `cells`, `fromseed` | Motorola P25 talker-alias recovery (flagship) |
...
| `crc` | `recover`, `compute` | reverse-engineer / compute CRC parameters |
...
## Flagship: Motorola talker-alias recovery

The alias cipher is a length-seeded, keyless, byte-oriented obfuscator. The
framing, the recovered output substitution table, and the per-character
decode `char = int8(Modd·(LUT[eo] − Hodd))` are established; the unknown is
the per-character state update. The toolkit provides four incremental,
logging, resumable recovery modes over a user-supplied ground-truth corpus
(`rid,talkgroup,encoded_hex,alias`; the trailing 2-byte CRC is stripped
automatically):
...
The two state bytes form 256×256 = 65,536-cell functions; the passive corpus
exercises only a few hundred cells of each. The **high byte is readable from
ciphertext alone**, so the `structure`/`fromseed` modes can keep chipping at
it from passive captures with no transmitter. The **multiplier** byte is only
touched by labeled rows, so the `cells` mode improves monotonically as more
labeled data arrives. Each mode's report ends with what additional data would
close the remaining gap — the effort is always directed, logged, and
resumable.
