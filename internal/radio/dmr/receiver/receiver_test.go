...
// / 4800 baud), and the SymbolSink is never fired on the pure-C4FM DMR path.
func TestReceiverEmitsSoftAndEyeTaps(t *testing.T) {
	iq := makeC4FMIQWithOffset(balancedStream(600), 0)
	...
	if symbolFired {
		t.Error("SymbolSink fired on the pure-C4FM DMR path (it should never fire)")
	}
}
...
func TestReceiverTapsDoNotAlterDecode(t *testing.T) {
	iq := makeC4FMIQWithOffset(balancedStream(400), 37)
	plain := decodeDibits(iq)
