...
var fieldMetas = map[string]FieldMeta{
	...
	"RecordingsConfig.TETRALMSEqualizer":    {Label: "TETRA LMS equalizer", Help: "Opt the TETRA voice chains into the per-burst training-sequence LMS equalizer (trains on each burst's NTS midamble) on top of the receiver's blind CMA. Off by default — this is the capture-gated on-air A/B lever for the concurrent-load garble, not a confirmed win. A/B on a same-carrier site before leaving it on."},
