...
var fieldMetas = map[string]FieldMeta{
	...
	"RecordingsConfig.SpecAmplitudeEnhance": {Label: "Spec-faithful amplitude enhancement", Help: "Restore the π/ω₀ factor in the §6.2 spectral-amplitude enhancement (per TIA-102.BABA / mbelib) that the legacy form dropped, lifting high-band harmonics off the attenuate clamp — clearer DMR and P25 voice, most audible on higher-pitched speakers. Unset defaults ON; set false to A/B against the legacy formula. Codec-correctness fix, not a tone preference."},
