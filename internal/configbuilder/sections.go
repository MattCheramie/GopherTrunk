...
func Sections() []SectionMeta {
	return []SectionMeta{
		...
		{"display", "Display", "Display",
			"How the daemon renders timestamps for humans (the decoded-message log, power log, and TUI). Does not affect the JSON/gRPC API, webhooks or rdioscanner uploads, which stay UTC RFC3339 for interoperability.",
			"Display", docsBase + "web.html"},
		...
		{"fleetsync", "FleetSync", "FleetSync",
			"Kenwood FleetSync FFSK signalling receiver. Target the conventional analog voice channels your Kenwood fleet uses — the fleet / unit ANI burst rides at the head of each transmission.",
			"FleetSync", docsBase + "fleetsync.html"},
		{"adsb", "ADSB", "ADS-B",
