...
func TestFleetSyncReplay(t *testing.T) {
	...
	// Resolve the carrier: an SDR# / SDR++ baseband recording is the whole
	// tuner span with the signal wherever the VFO sat relative to the centre
	// (the #1184 captures: +424.5 kHz and +323 kHz off centre at 2.048 MS/s),
	// so with no GT_FLEETSYNC_TUNE_HZ a wideband capture is searched for its
	// carriers and each is tried, strongest first.
	if os.Getenv("GT_FLEETSYNC_TUNE_HZ") == "" && capt.audio == nil && capt.rate > fleetSyncDDCTargetHz {
		tuneHz = searchFleetSyncCarrier(t, capt, bauds[0])
	}
...
// fleetSyncCarrierCandidate is one carrier the averaged spectrum found.
type fleetSyncCarrierCandidate struct {
	offsetHz float64
	aboveDb  float64 // above the spectrum's median floor
}

// fleetSyncCarrierCandidates ranks the carriers in a wideband capture by
// averaged power: a Hann-windowed 4096-point periodogram averaged over the
// whole scan span, local maxima at least 6 dB over the median floor, the
// strongest kept with a 25 kHz exclusion around each. It averages over
// SECONDS, deliberately: the shared dsp carrier estimator scans only its
// first ~256 short windows (tens of milliseconds), which on the #1184 SDR#
// captures is before the radio keys up — it ranked a -423 kHz spur first
// and never saw the +424.5 kHz FleetSync carrier that dominates the file.
func fleetSyncCarrierCandidates(iq []complex64, rate float64, maxCandidates int) []fleetSyncCarrierCandidate {
	const n = 4096
	if len(iq) < n {
		return nil
	}
	...
	for off := 0; off+n <= len(iq); off += n {
		for i := 0; i < n; i++ {
			s := iq[off+i]
			in[i] = complex(float64(real(s))*win[i], float64(imag(s))*win[i])
		}
		out = plan.Forward(out, in)
		for k, v := range out {
	...
	for k := range psd {
		db[k] = 10 * math.Log10(psd[k]/float64(blocks)+1e-30)
		sorted[k] = db[k]
	}
	...
	freqOf := func(k int) float64 {
		...
		return float64(k) * rate / n
	}
	type pk struct{ k int }
	var peaks []int
	for k := 0; k < n; k++ {
		prev, next := (k+n-1)%n, (k+1)%n
		if db[k] >= median+6 && db[k] >= db[prev] && db[k] > db[next] && math.Abs(freqOf(k)) <= 0.45*rate {
			peaks = append(peaks, k)
		}
	}
	sort.Slice(peaks, func(a, b int) bool { return db[peaks[a]] > db[peaks[b]] })
	var cands []fleetSyncCarrierCandidate
	for _, k := range peaks {
		f := freqOf(k)
		tooClose := false
		for _, c := range cands {
			if math.Abs(c.offsetHz-f) < 25_000 {
				tooClose = true
				break
			}
		}
		...
		cands = append(cands, fleetSyncCarrierCandidate{offsetHz: f, aboveDb: db[k] - median})
...
// searchFleetSyncCarrier finds the carriers in a wideband capture, tries the
// front end on each — strongest first — and returns the offset of the first
// that yields a CRC-valid burst (0 when none does, which then runs at
// centre). An SDR# / SDR++ baseband recording is the whole tuner span with
// the signal wherever the VFO sat relative to the centre, so this is what
// makes the harness a one-command run on such a file.
func searchFleetSyncCarrier(t *testing.T, c *fleetSyncCapture, baud int) float64 {
	t.Helper()
	...
	for i, cand := range cands {
		res := runFleetSyncChain(t, c, baud, cand.offsetHz)
		t.Logf("carrier search: candidate %d at %+.0f Hz (%.1f dB over the floor): sync_locks=%d crc_ok=%d",
			i+1, cand.offsetHz, cand.aboveDb, res.framer.BurstsIn, res.crcOK)
		if res.crcOK > 0 {
			t.Logf("carrier search: tuning to %+.0f Hz (set GT_FLEETSYNC_TUNE_HZ to override)", cand.offsetHz)
...
func TestFleetSyncReplayHarnessSelfCheck(t *testing.T) {
	...
	t.Run("wideband cs16, carrier found by search", func(t *testing.T) {
		...
		c := &fleetSyncCapture{format: "cs16", rate: rate, iq: synthFleetSyncIQ(t, rate, off)}
		got := searchFleetSyncCarrier(t, c, 1200)
		if math.Abs(got-off) > 500 {
			t.Fatalf("carrier search found %+.0f Hz, want %+.0f", got, off)
		}
		assertFleetSyncRun(t, runFleetSyncChain(t, c, 1200, got))
	})
